...
- **`main`**：`e5fb15c`（2026-09-23，启动画面淡出放缓 #127 已上线）。
...
- 所以是**你手机上那个图标**。iOS 只在「添加到主屏幕」那一刻读启动图，之后不再更新；你的图标是
  ...
  → **待你操作**：删掉图标 → Safari 打开 wealthup.cc → 添加到主屏幕 → 重开。还黑的话告诉我机型。
- 这条销掉了 #94 遗留的那句 pending（re-add icon for new startup image）—— 一直没做，现在知道
  它的代价是每次启动都先黑一下。
