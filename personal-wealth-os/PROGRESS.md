...
- **`main`**：`47b8fc3`（2026-09-22，PR #104 L-5 已合并上线）。
...
- **L-6 完成（2026-09-22，分支 `l6-debt-form`，未合并）**：Settings 和 Overview「写下欠多少」的债务表单改成选项式。先选种类，只出现那种债的问题，每题都是按钮、最后一个才自己填；信用卡只还一部分时才问利率（15% / 17% / 18%，Bank Negara 分级）；BNPL 问是否准时还（准时 = 不收利息）；名字自动用种类名；最低还款 / 月供自动算；底部一行即时说这笔债是先还还是照时间表还、每月多少、哪个月还清。Overview 面板照问答预先选好（种类、金额、是否全额还清）。填错时的提示一改答案就消失。
