...
- **未解决（2026-09-18）**：PR #78 上线后你**还是看不到**股息卡片。原因未查明，只在 demo 验证过。
  下一步需要：(1) 你在哪里看（wealthup.cc / localhost:5174 / 手机 App）；(2) Portfolio 页 Holdings 下面的截图。
  可能原因：看的是没更新的 App；卡片在但显示 "No payouts found"（例如交易日期格式不是 YYYY-MM-DD，除息日前算成 0 股）；或另一个问题。
- **当前**：查上面这个。之后可选：手动新增股息、新市场的目标比例、编辑交易、手续费自动估算。
