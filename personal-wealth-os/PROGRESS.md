...
- **`main`**：`868e533`（2026-09-23，启动画面白条修复已合并上线）。
- **工作方式**：见 `CLAUDE.md`（每次会话自动加载）。文档 bookkeeping 直接进 main，代码走 PR。
- **当前在做**：双端差异账本（电脑版 ↔ 手机版补齐），T1 / T2 已上线，下一个 T3。

## 启动画面底部白条（iPhone，已上线 PR #120）

- **现象（你在 iPhone 上发现）**：浅色主题下，开机那屏（深色 + 白色 W）底部有一条米白横条；
  深色主题看不出来。
- **原因**：`#launch` 是 `position: fixed; inset: 0`，只盖住网页视口。iPhone「加到主屏幕」的
  独立模式里，Home 指示条那条安全区不归页面元素画，iOS 拿 `<html>` 的背景色去涂 —— 而
  `index.html` 写死了 `html[data-theme='light'] { background: #f4f1ea }`。深色主题是
  `#0b0b0c`，接近黑所以看不出来。Safari 普通网页和安卓都不会有这条。
- **改动**：启动期间 `<html>` 固定 `#141310`（和启动画面同色）；遮罩退场时才加
  `html.launch-done`，把画布交还主题色（找不到遮罩元素时立刻交还，不会卡深色）。
  SW 缓存 v21 → v22，免得旧 app shell 继续供旧 `index.html`。
- **验证**：`npm run build` 通过；构建产物用无头 Edge 实测 —— 启动中浅色/深色都是
  `rgb(20,19,16)`，退场后浅色恢复 `rgb(244,241,234)` 且遮罩已移除。生产已确认
  （`html { background: #141310 }` + `sw.js` v22）。
- **待人工确认**：桌面复现不了 iOS 独立模式的安全区，需要你在 iPhone 上重开主屏图标看白条是否消失。
- **教训**：全屏遮罩盖不住 iOS 独立模式的安全区；那块由 `<html>` 背景决定，主题色必须一起管。
