...
- **`main`**：`bcdc3e0`（2026-09-22，PR #105 L-6 已合并上线）。
...
- **L-7 完成（2026-09-22，分支 `l7-savings-to-debt`，未合并）**：还债阶段存款比 1 个月开销多时，Overview 多一个可选步骤「Put MYR X of your savings on <债>」+ 每月省多少利息；按「I've paid」存款和债同时减少。问答高利率的回话也提一句。1347 测试全绿；无头 Edge 走过（存 2,656、留 664、还 1,992 → 债 3,000 → 1,008，还清提早到 2027 年 2 月），0 个错误。
- **下一步：L-7 的 PR 等你检查**。之后 L 系列全部完成。
