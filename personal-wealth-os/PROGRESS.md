...
- **`main`**：`1a3d55d`（2026-09-22，PR #103 L 系列已合并上线，Schema v29）。
...
- **带你用一遍（2026-09-22）**：走完问答（信用卡 / 车贷 flat / 全额还清）→ 结果页 → Overview →「I've paid」。真实帐号没有记过债，跳过最后一步。
  ...
  - 用的时候发现两个 bug → L-5。
- **L-5 完成（2026-09-22，分支 `l5-debt-fixes`，未合并）**：问答结果页的还清月份现在也算利息，跟 Overview 一样（你的例子：January 2028）；安全垫已存满的人，Your plan 卡照常显示「Safety buffer 1,992 / 1,992」。1336 测试全绿，无头 Edge 用你的例子走过一遍，0 个错误。
- **下一步：L-5 的 PR 等你检查；之后 L-6（选项式债务表单）**，等你说开始。
