...
- **`main`**：最新（含 V1-1、V1-5；V1-2+V1-3 在 PR #15 待合并）。
...
- **V1-2 + V1-3 完成**：云同步不再靠设备时钟 —— 改用 `onSnapshot` 的 `hasPendingWrites`；另一台设备改了 → 通知 + Reload（PR #15，待合并，多设备手验待做）

## 当前正在做

- V1-2+V1-3 收尾（PR #15）。等它合并 + 你做多设备手验。

## 下一步

1. 合并 PR #15，做多设备手验（两个浏览器 profile @ 5173）。
