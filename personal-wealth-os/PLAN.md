...
### T1 — Portfolio 手机版：未实现盈亏金额 + 最近成交时间  `[x]`（PR #117）
### T2 — Review 电脑版：真正的纪律分趋势图（底槽 + 月份标签）  `[x]`（PR #119）
### T3 — Budget 手机版：额外接住、每层填充条与说明、百分比层规则  `[ ]`
...
## 启动画面淡出放缓  `[x]`（2026-09-23，PR #127 已合并上线，`main` = `e5fb15c`）

用户报：「从 logo 去到 overview 的画面还会闪一下」。#125 默认改浅色之后出现的。

- 根因不是 bug，是设计冲突：**启动画面在两个主题下都是深色** `#141310`（#93 / #94 定的，
  iOS 31 张启动图和安卓 splash 是烘焙好的图片，运行时改不了主题）。以前 App 是深色所以看不出
  接缝，现在是深色 W 淡出到接近纯白的 Overview，220ms 走完这么大对比 = 闪。
- 选了方案 A（最小代价）：淡出 `220ms` → `450ms`。B（整套启动图改浅色）会让深色主题用户反过来
  闪；C（默认改回深色）等于撤销 #125；D（只让网页启动画面跟主题走）解决不了 —— iOS 上闪的位置
  只是从「网页→App」挪到「系统图→网页」。**根本矛盾：系统级启动图不可能知道 App 内的主题。**
- 埋着的雷：移除遮罩的安全定时器写死 `400ms`。淡出一调到 450ms，它就会在淡完之前把遮罩硬拔掉，
  制造一次新的跳变。改成从元素上读 `getComputedStyle(launch).transitionDuration + 250ms`，
  以后改 CSS 一处就够。减弱动效（`transition: none`）下等待 400ms → 250ms。
- **没升 SW 缓存版本**（见下面「教训」）。

## 黑屏 → logo → Overview（**未解决，等用户重加主屏幕图标**）

用户同一轮报的第二件事，跟上面**不是同一段**：黑屏在 logo **之前**，即 `index.html` 还没到，
网页一个像素都画不了，主题和淡出都管不着。

- 量过，不是我们代码晚：W 是内嵌 data URI，**跟 HTML 同一刻出现，0 延迟**
  （暖启动 `#launch` 进 DOM 153ms / W 可见 153ms；冷启动 300ms / 382ms）。
- 线上这一侧是好的：31 个 `apple-touch-startup-image` 链接都在、图片 200 且能下到、
  manifest `background_color` = `#141310`、覆盖 iPhone 6 → 17 + Air。
- 所以是**用户手机上那个图标**：iOS 只在「添加到主屏幕」那一刻读启动图，之后不再更新。
  用户的图标是 #93 / #94（2026-09-19）之前加的，记住的是「这个 App 没有启动图」。
  → 已请用户删掉图标重新添加。还黑的话要问具体机型，核对 31 张图里有没有那个尺寸
  （今天 2026-09-23，今年的新机可能没覆盖）。
...
### Task 2 — `estimateUsSellFeeMyr` + snapshot 字段 + 测试  `[x]`
### Task 3 — 画面：Unrealised 显示卖出后金额，Fees 显示预估卖出手续费  `[x]`

FUTURE IDEAS：
- 马股、港股、新加坡、伦敦的卖出手续费（用户已贴收费表），等有真实订单可以核对时再加。
- 标出 ETF / 股票，印花税上限才能分对（ETF MYR 200，股票 MYR 1,000）。
...
用户的看法（2026-09-22）：利率太高的债应该先还，不应该先存 buffer 和投资；请 Claude 研究。

**研究结论**（来源见下）
- 还掉 RM1 的 18% 信用卡欠款 = 稳拿 18% 回报；FD / 储蓄 / MMF 约 3%，股票长期约 6–8% 且不保证。债的利率高过能赚的回报，就先还债。美国常用门槛约 6%。
- 马来西亚信用卡（Bank Negara 分级）：最高 15% / 17% / 18%；最低还款 = 欠款 5% 或 RM50，没还清的部分下一期开始算利息。
- 常见顺序：所有债照付最低还款 → **小额应急钱** → 集中还高利率的债（利率最高的先还）→ 存满安全垫 → 投资。小额应急钱是为了不让一次急用又刷回信用卡。
- 低利率的债（PTPTN 约 1%、房贷约 4%）照时间表还，多出来的钱存安全垫和投资比较划算。
- 个人贷款常写 **flat rate**：flat 6% ≈ 真实年利率约 11%，直接用会把高利率判成低利率。
- 最低还款都付不起：AKPK（Bank Negara 设立，免费）可以帮忙跟银行谈、降低或免除利息。
- 来源：Bankrate、Bread Financial、Northwestern Mutual、AFS（先后顺序和门槛）；Bank Negara、RinggitPlus（信用卡分级利率）；money.com.my（AKPK）。

**已定的决定（2026-09-22，照建议）**
1. 高利率门槛 **8% 真实年利率**（马来西亚安全回报约 3–4%，比美国的 6% 高一点）。
2. 小额应急钱 = **1 个月开销**；不知道开销时 **RM1,000**。
3. 另开 L 系列，不塞进 Q 系列的 PR。Q 系列先上线，那时的还债支线还没按利率分（已写在 Q-5）。

**按利率分三级**
| 真实年利率 | 例子 | WealthUp 怎么做 |
|---|---|---|
| 高（≥ 8%） | 信用卡、BNPL 逾期、大部分个人贷款 | 先存 1 个月开销的小额应急钱，然后多出来的钱**全部**还债，利率最高的先还；这段时间不存满安全垫、不投资 |
| 中（4–8%） | 车贷 | 照时间表还；照常存满安全垫，之后多出来的钱投资和提早还一半一半 |
| 低（< 4%） | PTPTN、房贷 | 照时间表还；不进还债阶段 |

### L-1 — 利率分级 + 阶段（纯逻辑，不改画面）  `[x]`（2026-09-22，分支 `l-debt-rates`，L-4 时一起开 PR）
- `moneyStage.ts`：还债阶段只在「有高利率的债」时出现（记下的负债 `annualRate ≥ 8` 且还有欠款；或问答说了高利率的债但还没记下），分两小段：先存小额应急钱（1 个月开销 / RM1,000），再全力还债。高利率的债还清 → 回到存安全垫。低利率的债就算问答选了还债，也走一般阶段，原因写「这笔利率低，照时间表还，先存安全垫比较划算」。
- `onboardingQuiz.ts` 的每月分配：有高利率的债时，小额应急钱没满前 安全垫 50 / 还债 50；满了以后 还债 100、投资 0。
- 测试：三级利率各一种、刚好 8%、小额应急钱的边界、还清后回到一般阶段、多笔债取利率最高的、没有利率数字时的处理。

### L-2 — 问答：债的种类、利率和期限（Schema v29）  `[x]`（2026-09-22，分支 `l-debt-rates`，L-4 时一起开 PR）
- **2026-09-22 用户补充（已做）**：债通常有期限 → 有期限的种类问「还剩几年」，负债存「还清月份」`endMonth`；月供由欠款 + 真实年利率 + 剩几个月算出；flat 换算用真实期限（不填按 5 年）。信用卡问「每个月都全额还清吗」，全额还清的不收利息、不进还债阶段（`paidInFull`）。选 pay off debt 的新用户在问答里就会被问到这些。
- **顺便修的旧 bug**：利率单位统一成小数。表单输入百分比、存小数；v29 迁移把旧数据 ≥ 1 的利率除以 100（旧版本存的都带 v28，下次载入会再修一次）。
- 种类 / 利率 / 期限 / 全额还清已放进 Settings 负债表单和 Overview「写下欠多少」面板（共用 `components/liabilityForm.ts`），原计划 L-3 的这部分提前做了。
- 以下是原计划：
- 还债那句改成「The debt is [a credit card ▾] of [RM3,000 ▾]」：种类（信用卡、个人贷款、BNPL、车贷、PTPTN、房贷、其他）带入常见利率（信用卡 18%、PTPTN 1% 等，标 estimate），可以自己改。个人贷款问是 flat rate 还是真实年利率，flat 自动换算（写明是估算）。
- `onboardingAnswers` 加 `debtKind`、`debtRate`（v29，迁移同 v28 的做法）；Start my plan 时如果有金额，直接记成一笔负债（带利率），「写下欠多少」那步就只剩核对。
- 回话：高利率「每月利息约 RM X，先还它最划算」；低利率「利率低，照时间表还就好，先存安全垫」。

### L-3 — Overview：还债的下一步和面板  `[x]`（2026-09-22，分支 `l-debt-rates`，L-4 时一起开 PR；面板部分已在 L-2 做完）
- 下一步：先存小额应急钱 → 「先还利率最高的那笔：Credit card 18%」+ 每月还款金额；开销 ≥ 收入、算不出还得起多少时，一行提示 AKPK（免费）。
- 「写下欠多少」面板加种类 / 利率（flat rate 换算同 L-2）；Settings 的负债表单也标明「真实年利率」。
- Your plan 卡：有高利率债时显示「还完约 X 月」和每月利息。所有说明标「仅供参考，不是财务建议」。

### L-4 — PR → 用户检查 → 用户说可以才合并 → 上线 → 文档  `[x]`（2026-09-22，PR #103）

### L-5 — 带用户用一遍时发现的两个 bug  `[x]`（2026-09-22，PR #104 已合并上线，`main` = `47b8fc3`）
- 问答结果页的还清月份没算利息：RM3,000、18%、每月 RM216 显示 November 2027，Overview 显示 January 2028。现在两边用同一个算法（`monthsToClear`），都是 January 2028。
- 已经存满安全垫的人，Your plan 卡却显示「Emergency money, for now 664 / 664」。现在只有还没存满时才显示「for now」。

### L-6 — 债务表单改成选项式  `[x]`（2026-09-22，PR #105 已合并上线，`main` = `bcdc3e0`）
- 先选种类（按钮），再按种类出现不同的问题，每题都是选项、最后一个才自己填（跟问答一样，共用 `DEBT_KINDS`）。
- 信用卡：欠多少 / 每月全额还清或只还一部分 / 只还一部分才问利率（15%：12 个月都准时、17%：10 个月准时、18%：其他、自己填）。车贷、个人贷款：利率选项（预设 flat）+ 还剩几年。PTPTN：1% / 3%（2008 年前）。房贷：3.5 / 4 / 4.5%。BNPL：0%（准时还）/ 18%。其他：名字自己打、利率「不清楚」。
- 名字自动用种类名；最低还款 / 月供自动算、显示为估算，不再要用户填；底部一行即时显示真实年利率和还清月份。Settings 和 Overview 面板共用。资料格式不变（v29）。

### L-7 — 多出来的存款拿去还高利率的债（原 FUTURE IDEAS）  `[x]`（2026-09-22，PR #106 已合并上线，`main` = `e6cdccd`）
- 还债阶段、第一笔债利率确定 ≥ 8%、存款比 1 个月开销多出 RM50 以上时：Overview 多一个可选的下一步「Put MYR X of your savings on Credit card」，写明每月省多少利息；按「I've paid」时，存款和那笔债同时减少。金额最多到欠款为止。
- 问答里高利率的回话也加一句：「你存了约 RM X，留 RM Y、把 RM Z 拿去还，每月省约 RM W 利息」。

## 问答后的 Overview 整理 + 填空式问答 + 分阶段（Q 系列）  `[x]`（2026-09-22，PR #102 已合并上线，`main` = `6fbe3f9`）

问题：O 系列问答做完后，Overview 第一屏有 9 块差不多大的东西，看不到重点——目标出现两次（Financial goal 卡 + 计划条「拍立得 No date yet」）、当前步骤出现两次（大标题 + 清单）、两个绿框（✓ Pay recorded、Because）抢眼、「No date yet」是死胡同，Net worth 被挤到最下面。
另外，用户想学竞争对手 Versa 的问答：像填空造句（「I'm [a Student ▾]」）、用范围不用精确数字、先讲道理再问、问「存了几个月」和「钱放在哪」、分短期 / 长期目标，然后按答案把用户分类。
示意页（v2，含底部面板）：https://claude.ai/artifact/CPgmdEtLygu466XZYL7CHa

**已定的决定（2026-09-22，用户回「ok」，照建议）**
1. 下一步的主按钮在 Overview **底部弹出面板**，表格预先填好，按 Save 才存；存到原本的地方（Ledger / Settings / Portfolio），用同一套检查规则。提示条「Saved to Ledger · View」。「转钱进安全垫」照旧一键确认；Money leaks、规划投资这类工具照旧跳页。
2. 金额用**范围 + 「Type an amount」**；计划用范围中间值，标 `estimate`。
3. 安全垫目标：一般 3 个月，**自雇 6 个月**。
4. 不问性别、种族；土著 / ASB 放 FUTURE IDEAS。
5. **阶段不存**，每次从真实数据算（问答答案只是起点），老用户不用重做问答也有阶段。
6. 先做 Q-1。

**Overview 改后的样子**：阶段一行（替换「Safety buffer: 0% funded」）→ Your plan 卡（目标只出现一次；没日期时换成可以点的下一步）→ Your next step 卡（圆点「2 of 3」、标题、灰字 Because、主按钮 + Later；清单和 Hide 收进「All steps」）→ Net worth / This month 回到第一屏。「✓ …做完」改成 4–5 秒后消失的提示条。

**4 个阶段 + 还债支线**（按顺序检查）：
| 阶段 | 条件 | 下一步 |
|---|---|---|
| 还债支线 | 主要目标 = 还债 | 记收入 → 记成负债 → 每月固定还 |
| 1 打底 | 开销 ≥ 收入，或存款 < 1 个月开销 | 记收入 → 记开销 → 找一项可以省的 |
| 2 存安全垫 | 1 个月 ≤ 存款 < 目标月数 | 记收入 → 转钱进安全垫 → 记开销 |
...
- 不改数据结构，不升 Schema。这一步还没有阶段（阶段一行等 Q-2）。
- 验证：typecheck / 测试 / build；5199 `?fresh` 走完问答 → 面板完成三步；深浅色、电脑 + 390 手机；无溢出、无报错。你在 5174 确认真实账号看不到卡片、数字不变。

### Q-2 — 阶段分类  `[x]`（2026-09-22，分支 `q-overview-tidy`）
- 已做：`src/moneyStage.ts` `classifyStage(state)`，按顺序：还债支线 → 开销 ≥ 收入 → 安全垫满没满 → 有没有交易。
  - 月开销用 Settings 的固定开销（问答也写在这里，跟安全垫建议同一个数）。
  - 「开销 ≥ 收入」看**上个完整月**的记账（有收入记录时），没有才看计划收入；这个月还没发薪不会被当成超支。
  - 安全垫满：有目标看目标；没目标时 3 个月开销算满。只有目标没有开销数字时，用目标的百分比说明。
  - 还债支线：问答选了还债，直到记下的负债还清为止。
- Overview 标题下面那句「Safety buffer: 0% funded」换成阶段一行（4 格 + 原因）；那句话 Health 卡里照样有。**老用户也会看到**（demo 账号是「Stage 4 of 4 · Growing」）。
- 顺手改：Your plan 卡在的时候，Financial goal 那一行都先不显示（问答写的那句话要么是目标、要么就是安全垫，卡片已经写了）。
- 实测（5199，独立无头 Edge，手机深色）：demo 账号、问答跳过开销、开销 2800 / 存款 3000、选还债，四种阶段一行都对，无报错。12 个新测试，1262 全绿，typecheck / build 通过。
...
### Q-5 — 阶段决定下一步顺序  `[x]`（2026-09-22，分支 `q-overview-tidy`）
- 已做：`buildNextSteps` 先算阶段，再按阶段排顺序（表里的排前面，其余照原来的顺序跟在后面）：
  - 还债：记薪水 → **写下欠多少**（底部面板，存到 Settings 负债，检查跟 Settings 一样，另外欠款要 > 0）→ **每月还 MYR X**（按「I've paid」确认，从最大的那笔负债扣，不会扣到负数；X = 问答给这笔债的每月金额，没有就不出现这一步）→ 记开销。负债低于问答说的金额就算还过。
  - 打底（真的超支）：记薪水 → 记开销 → **找一项可以省的**（跳到 Money leaks；没有数据能判断「找过了」，所以是可选，不会挡住卡片完成）。
  - 存安全垫：记薪水 → 安全垫目标 → 转钱进安全垫 → 记开销。
  - 可以投资：记薪水 → 记开销 → **定每月投资多少**（底部面板，存到 Settings 的 Monthly DCA，同步 dca 规则；完成 = `dca.monthly > 0`）→ 第一笔交易（可选）。
  - 投资中：记薪水 → 第一笔交易 → 记开销。
- 阶段加了 `basis`（根据什么判断）：「不知道开销」的第 1 阶段不是真的钱不够，所以不套「打底」的顺序、也不出现「找一项可以省的」——全新账号的顺序跟以前一样。
- 实测（5199 `?fresh`，独立无头 Edge，手机深色，用新问答走）：还债（面板预填 Credit card / 3000、负利率被拒、存进负债、按 I've paid 后 3000 → 2250、阶段一行跟着变）、可以投资（面板建议 MYR 1,400、存进 dca.monthly）、超支（顺序对、Money leaks 是可选）。无报错。8 个新测试，1281 全绿，typecheck / build 通过。
- 还债进度条（用户要求，开 PR 前改好）：问答的还债目标在 Your plan 卡显示「2,250 left of 3,000」，进度条按已还的比例，日期写「Cleared around …」；已还 = 问答说的金额 − 记下的负债（没记负债时用目标自己的数字，欠得比说的多不会变负数）。按「I've paid」时同一笔钱也加进那个目标的进度，Goals 页和 Next goal 卡片一致。2 个新测试，1283 全绿；5199 实测 3000 → 付 750 → 卡片「2,250 left of 3,000」、目标 750 / 3,000。
- **还没按利率分**：现在的「还债支线」只看问答选没选还债，不看利率——PTPTN 1% 也会进来，信用卡 18% 也还是会被建议存安全垫。按利率分级是 L 系列（下面）。

### Q-5（原计划）
- `buildNextSteps` 按阶段排步骤（见上表）；还债支线加「记成负债」「每月固定还」。

### Q-6 — PR → 你检查 → 你说可以才合并 → 上线 → 文档  `[x]`（2026-09-22，PR #102）
