...
## 问答后的 Overview 整理 + 填空式问答 + 分阶段（Q 系列）  `[ ]`（2026-09-22 定）
...
- **已知、没改**：还债的人，Your plan 卡里的「Credit card 0 / 3,000」是问答当成储蓄目标建的（O-1 起就是这样），还钱后负债变少，但这条进度不会动。要不要改成显示负债的进度，等用户决定。

### Q-5（原计划）
- `buildNextSteps` 按阶段排步骤（见上表）；还债支线加「记成负债」「每月固定还」。

### Q-6 — PR → 你检查 → 你说可以才合并 → 上线 → 文档  `[ ]`
