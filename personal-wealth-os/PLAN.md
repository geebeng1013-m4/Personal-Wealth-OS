...
## 迁移补空缺不再用示例数据  `[x]`（2026-09-25，PR #159 已合并上线，`main` = `5c14d88`）

来自 `PLAN.md` 的 FUTURE IDEAS（原 R-4 C 条）。用户 2026-09-25 从整份待办里挑中这一条。

**问题**：`migrateState()`（`src/state.ts`）是所有数据进入 App 的必经之路——本地读取、导入文件、
云端文档都走它。它补缺的基底是 `cloneDefaultState()`，而 `defaultState` **不是空状态，是示例状态**。
所以一份缺了某块的数据会安静地长出：紧急基金 4,000/4,000（显示「已存满」）、每月开销 880/400/320、
熊市储备 400 加三档、定投每月 100（VOO 70% / QQQM 30%）、`profile`「Student Investor」19 岁，
以及六层示例 `buckets`（`buckets` 连显式赋值都没有，是靠开头那句展开漏进来的）。
更常见的是**半块缺**：只给 `{ current: 500 }`，target 变 4,000，进度条立刻显示 12.5%。

违反 `AGENTS.md` §5「缺失字段使用明确默认值」——现在的默认值是演示用的钱。而且它不报错：
界面完全正常，只是底下所有建议、健康分、抄底阶梯都建立在假数字上。

**更要紧的一处**：`loadState()` 在本地没有数据、或存着的文字解析失败时也回落到示例状态，
而 `main.ts:501` 在登录后**先渲染本地这份**再等云端。所以**新设备 + 云端读取失败/离线** ＝
用户看着 22 笔示例交易操作，一旦编辑就会被 `saveState` 当成自己的存下去并同步上云。

**做法（Task 1–4）**：
1. 先写会失败的测试，把每一块该有的空值钉住（7 红 2 绿，绿的是安全网）。
2. `emptyState()` 拆成 `emptyStateBase()` ＋ 设备标识；基底**不碰 `deviceId()`，因此不碰 localStorage**
   （迁移每次加载都跑，Node 里没有 localStorage）。六处兜底改用基底，`buckets` 补 `validBuckets()`
   （逐条丢掉坏层，保住其余），pre-v15 那条**故意**用示例交易补齐的分支原样保留。
3. `loadState()` 两处回落都改成空态；解析失败打日志不吞，**原文留在原地**可救。
4. 真实数据验证。

**不升 version**：字段没有增删改名，变的只是「缺失时拿什么补」，从不改动已存的值。

**验证**：
- 拿 `main` 的 `migrateState` 和分支的**同时跑同一批完整状态**（demo 完整态、二次迁移、旧版本号、
  无交易），逐字段深比对：**四例全部 identical**。同一支脚本打印出预期中的差异：
  `emergency 4000/4000 → 0/0`、`buckets 6 层 → 0 层`、`DCA 100 → 0`、`cashflow 880/400/320 → 0/0/0`、
  `profile "Student Investor" 19 → "" 0`。
- 往 demo 存储键塞一份只有 `version` 的状态再刷新：进入**引导流程**（「Let's build your money plan」），
  控制台 0 错误——正是 O 系列为这种情况做的东西。
- `npm test` 1472/1472（新增 11 条）、`typecheck`、`build` 全过。

**两个老测试是把 bug 当前提，已改成自述前提**：`budgetSummary` 的 `plannedState()` 原本借迁移送的
4,000 紧急基金（注释写着 "The **seeded** 4,000"），现在自己写明；`allocationPlan` 的
「每个旧版本都迁移出可用计划」喂的是空状态，靠示例 buckets 才有层，现在夹具自带层，并补了另一半：
没有层的状态得到空计划，`validatePlan` 报 `no-steps`——这正是 `emptyState()` 的产物，也是每个新用户
的起点，Budget 页早有对应空状态文案。

**线上核对**：本地构建 `index-P12QxSdQ.js` 与线上同名且逐字节相同，并 `curl` 到了新加的那句
`stored state could not be read`（函数名如 `emptyStateBase` / `validBuckets` 会被压缩改名，
**字符串不会**，所以用字符串当标记）。CSS 文件名没变是对的——这次只动 TS。

**未验（需要真实登录，本环境登不进去）**：真正的「新设备首次登录」与「真离线启动」。这两条路径落在
`loadState` 的两个分支上，各有一条测试钉着。**用户已在 5174 真实账号自查（2026-09-25）：紧急基金、每月开销、熊市储备与三档、
每月定投、Budget 层数、Me 页姓名年龄——「数字都没变」。** 也就是说他那份状态本来就是完整的，
这次改动对他是零影响，与并排比对量出来的「完整数据迁移前后 identical」一致。

## 快捷记账（K 系列）  `[x]`（2026-09-24 完成）

把每笔支出从「选类型 → 选分类 → 选账户 → 打备注 → 输金额」压成「点一个标签 → 输金额」。
决策与竞品对照见 https://claude.ai/code/artifact/JTmVFeSkP6763A1Ed4UC1q
（已定：模板全自动、金额不预填、完整备注与商家前缀混排、展开不聚焦金额、两种写法不合并）。

- `[x]` **K-1** 从历史算标签（`src/ledgerTags.ts`，纯函数 + 22 测试）— #149
- `[x]` **K-2** 表单按填写顺序重排，备注移出折叠，金额不再自动聚焦 — #149
- `[x]` **K-3** 标签条上屏 + 点一下填表 — #149
- `[-]` **K-4** 钉住 / 隐藏标签 —— **砍掉**（2026-09-24，用户决定）。按次数高到低排就够，
  不需要手动干预。顺带好处：K 系列因此**完全不碰持久化**，没有版本迁移的风险。
- `[x]` **K-5** 分类走偏才提示：仅当该商家 ≥5 次且同一分类占 ≥80% 时才吭声
  （`weige` 是 2:2、`chagee` 4:2，不设阈值会天天误报）— #151
- `[x]` **K-7** 「再记」：保存后不关表单，直接记下一笔（64% 的日子当天记 ≥2 笔，最多 12 笔）— #152
- `[x]` **K-6** 验证与文档收尾

另有一个上线后按实际观感提的顺序微调（Amount 移到 Category 下面、Note 移到标签下面）— #150。

**已知遗留**：手机上助手悬浮球会压到 Save Transaction 右边缘（原本就有，非本系列引入）。
...
### T3 — Budget 手机版：额外接住、每层填充条与说明、百分比层规则  `[x]`（2026-09-25，PR #155 已合并）

- 「额外接住」：电脑版有自己的数字卡，手机版只有一张当月卡，所以做成卡里的一行
  （`Extra caught +X · went to Y`，没有结余时说明「层填完没剩」）。
- 每层的填充条与说明（`X% filled · 备注`）手机上一并显示；备注太长时按目标备注的做法收起。
- 「百分比层加起来等于」那一行本来只在电脑版的 Plan rules 卡里，手机没有这张卡，
  所以在 layers 卡的列表末尾也渲染一份（`wu-budget-phone-only`）。
- 顺带：填充条原本整块 `aria-hidden`，把「X% filled」也一起读没了；现在只有条本身隐藏。
### T4 — Goals：手机补百分比，电脑补「还要几个月」  `[x]`（2026-09-25，PR #156 已合并）

- 手机：进度条那一行右端补百分比，与上方金额右对齐；已完成的目标在那里显示 `Reached` /
  `Done` 药丸，同时把名字底下重复的那个词去掉（一行不说两遍）。
- 电脑：`%` 与 `SAVED` 之间加一列 **TIME LEFT**（`13 mo`）。已完成、没目标、每月 0 的显示
  `—`，旁边 Monthly 列的 0 就是原因。数字仍是 `goalSummary.ts` 的 `estimatedMonthsToTarget`，
  没有新算法、不碰 state、不升版本。
- **901–1100px 顺带修好**：第七列会让目标名直接压到 Monthly 上（本来这个宽度就已经挤到
  名字要折三行）。该区间改成手机写法——名字底下带节奏说明，Monthly 与 Time left 让位。
### T5 — Market 手机版：下一档加仓价位、持有单位数、超配幅度  `[x]`（2026-09-25，PR #157 已合并）

电脑版四张卡每张底下那句话，手机上一句都没有。三句都补上，各自贴着它解释的数字：

- **下一档加仓价位**：放在手机价格行下面、`−1.0% from high` 药丸的正下方（右对齐）。
  一开始做成左对齐，用户说「为什么是往左的」——它解释的是右边那颗药丸，挂在左边就断了关系。
  不在 VOO/QQQM 梯子里的代码显示 `From its highest close`，与电脑版一致。
- **持有单位数**：position 卡 `Value` 下面。
- **超配幅度**：`+8.2%`，跟在 `63% / 55%` 下面。方向由卡片右上角的药丸用文字说，这里只说大小。

**position 卡改成两栏**（用户选的排法）：左＝持有多少（金额 + 单位），右＝跟目标比
（`63% / 55%` + drift）。先试过三栏两端对齐（Value 左、Weight 中、Target 右），用户说
「怪怪的」——一行里三种对齐、右下角还空一格。两栏各三行，两边都贴卡片内缘。
weight 与 target 合成一个数字后，`targetPlain` 这个 `data-market` 键没有读者，一并删掉。
...
退不出去 —— 见下面 FUTURE IDEAS。启动图链接因此放回（`LINK_STARTUP_IMAGES = True`）：
关掉它没换来好处，却让 logo 晚 100–150ms 才出现。开关和推理留着。

**登录页底部白边（我在 #125 弄坏的）**：登录页两个主题下都是深色 `#050706`，但画布跟着主题走 ——
`html.launch-done[data-theme='light']` 和 `body` 在浅色下都是 `#f4f1ea`，而 `.login-shell`
是 `#app`，够不到 iOS 的 home indicator 安全区。**和 #121 同一批表面、同一个教训**，所以
`main.ts` 早就为状态条准备了 `LOGIN_BG`；这次用 `<html>` 上的 `is-login` 把另外两个也堵上。
- **限定在 `.launch-done` 之后**：`renderLogin()` 在启动画面还在时就跑，不限定会把启动底色从
  `#141310` 拉到 `#050706`。实测已确认限定生效。
- 加在 `renderLogin`、摘在 `renderSignedIn` —— 这两个函数是通往各自画面的唯一入口
  （登出经由 auth 监听器回到 `renderLogin`）。
- 验证：登录页浅/深两次，`html`/`body`/`#app`/状态条**全部 `rgb(5,7,6)`**；启动全程底色保持
  `#141310`，退场后才切 `#050706`；已登录路径类从未设置，照旧 `#f4f1ea`。
- 未验：会话内 登录页 ↔ App 互切（demo 进不了登录页，真实构建登不进去）。

## 拿掉启动图链接  `[?]`（2026-09-23，PR #131 已上线，`main` = `fd06f33`，**待真机确认，需重加图标**）

#130 之后用户回「结果还是一样」，并问了关键一问：**为什么其他 App 没这个问题。**

**答案就是原因本身**：原生 App 只有一张图 —— 启动画面和第一个真实画面由**同一个渲染器、同一个窗口**
画出，iOS 的开场动画作用在整个窗口上，不存在对齐问题。网页 App 有**两张**：iOS 的启动图 PNG，
和 Safari 引擎画的网页启动画面，iOS 在两者之间交叉淡入淡出。**而 iOS 的缩放之所以看得见，
正是因为启动图里有 logo 可以看 —— 一块纯色在放大时看起来毫无变化。**

用户录屏实测（48.4fps，384×832）：
- 帧 52–57（120ms）：W 放大 16%、上升 21.5px ← iOS 从图标放大
- 帧 60–61（41ms）：整屏变亮 ← 交叉淡入淡出开始
- 帧 62–63（41ms）：第二个 W 在上方 20px 闪现 ← iOS 把淡出层继续缩放
- 帧 64+：稳定，中心 418.0，**与原生启动图完全一致**（#129/#130 是对的）

抖动发生在 **iOS 转场过程中**，不是转场之后，所以对齐做得再准也没用。

改法：拿掉 31 个 `apple-touch-startup-image` 链接 → iOS 用 manifest `background_color`
（`#141310`，与页面同色）铺屏 → 缩放纯色看不见 → 页面把 W 画一次，直接在最终位置。
- PNG 仍然生成在 `public/splash/`，整个改动是一个开关 `LINK_STARTUP_IMAGES`，改回 `True` 即还原。
- 代价：W 等页面画出来才出现（暖启动约 100–150ms），期间是同样的深色。
  **证据**：用户重加图标**之前**就是这个状态，他描述的是「黑」不是白 —— 说明 iOS 回退到深色填充。
- **必须重加主屏幕图标才生效**（iOS 只在添加那一刻读这份配置）。
- 若仍有跳 → 就是 iOS 交接机制本身，**停手**，记进 FUTURE IDEAS，不再让用户反复试。

## 去掉 W 的呼吸动画  `[?]`（2026-09-23，PR #130 已上线，`main` = `bf542f9`，**待真机确认**）

#129 之后用户回「虽然变快了，但还是看得出」。

- 量出来：**把动画停掉，网页和原生启动图相差 1px 以内**（W 360×209 vs 360×210，中心 1266.0 vs
  1265.5）。所以位置和尺寸已经对了，唯一还在动的是呼吸动画。
- 机制：原生启动图是**静止**的图，而 **iOS 不是在网页画出来那一刻就撤掉它的** —— 会多撑一会儿。
  等它撤时网页的 W 已经呼吸到 0.95 附近，于是 logo 忽然小一号再涨回来。
  （之前一张截图量到 W 高 201 而非 209 就是被动画污染的，201/209 ≈ 0.96。**以后量启动画面几何
  一定要先停动画**，否则数据是假的。）
- 两条路：延迟动画是**赌**（iOS 撑多久页面不可能知道）；去掉动画是把「有差别」本身消掉。用户选后者。
- 验证（逐像素比对 `apple-splash-1170x2532.png`）：底色区域最大差异 **0**；W 边界框
  (405,1162,764,1370) vs (405,1161,764,1370)；名字 1px 内；全屏差异 >8 的像素仅 **0.196%**，
  全部落在字形的抗锯齿边缘（PIL LANCZOS vs 浏览器缩放）。
- 代价：失去「正在加载」的呼吸感。暖启动 100ms 出画面、450ms 淡出，本来也看不清。
- **若真机仍看得出**：差别就不在 W 本身，而在 iOS 的交接方式（自带淡入淡出／中间闪一帧空白）。
  那种情况**需要用户的屏幕录影**才查得下去 —— 截图抓不到那一瞬间，录屏可以逐帧拆。

## W 按屏幕定位，对齐原生启动图  `[?]`（2026-09-23，PR #129 已上线，`main` = `6c93fb0`，**待真机确认**）

用户：白边没了，但「整体 W 往上」还在。**不是这两天改坏的，是 S-1 那天就有的偏差。**

- 为什么现在才看得到：重加图标之前 iOS 根本没显示原生启动图（那段黑屏），**没有原生图就没有东西
  可以「跳」**。这也是 S-2「逐像素比对，W 相差 1px 内」没抓到它的原因 —— 比的是图和图，
  不是图和真机上的网页。
- 测量：原生启动图 W 在屏幕正中（偏移 −0.5px）、名字底边 9.1%；用户截图上网页的 W 在屏心
  **上方 32.5px**、名字底边 **14.1%**。三个数互相印证 —— 白边高 64px，一半正好 32 = W 上移量；
  假设遮罩高 1216（1280−64）算出 W 中心应在 608.0，实测 607.5。
- 根因：`#launch` 是 `position: fixed; inset: 0`，iOS 独立模式下这个盒子比屏幕**短 65 设备像素**。
  W 用 `place-items: center` 居中在这个盒子里、名字 `bottom: 9%` 量这个盒子，而启动图是按
  **整块屏幕**算的（`generate-launch-images.py` 第 94 / 96 行）。两个基准不一样。
- 改法：W `top: 50vh` + `translate(-50%,-50%)`，名字 `9vh`，retry `22vh`。`#launch` 保持
  `inset: 0` 只管涂色，涂不到的地方 `<html>`/`<body>` 兜着（#121）。
  **`@keyframes launch-breathe` 必须带上 translate** —— keyframe 整个替换 transform，
  只写 `scale(0.95)` 会让 W 在每个呼吸周期一半的时间里掉到左上角。
- 验证：渲染 1170×2532 与 `apple-splash-1170x2532.png` 逐像素对齐（W 中心 1266.0 vs 1265.5，
  名字底边 9.1% vs 9.1%）；呼吸动画中途连续采样，宽度 120↔114.3 变化而 W 中心偏移恒为 0。
- **桌面验不到**：iOS 独立模式下 `vh` 是整屏还是同样的短盒子，只有真机知道（#121 同一个坑）。
  若真机仍跳 → 备用路是用 `env(safe-area-inset-bottom)` 把遮罩往下撑，需要用户新截图反推 env 实际值。

## 状态条颜色改为「启动画面退场后才写」  `[x]`（2026-09-23，PR #128 已合并上线，`main` = `9797a9d`）

用户重加主屏幕图标后截图：W 还在屏幕上，底部 home indicator 那条已经是浅色的。
**是 #125 弄坏的，而且正是 #121 修过的那条白线。**

- 取样（PIL 量用户截图，沿用 #121 的办法）：启动底色 `rgb(20,19,15)` = `#141310` ✅，
  底部那条 `rgb(245,242,236)` = `#f4f1ea`（浅色 `--surface-bg`），占屏高 5% = home indicator 安全区。
- #121 的结论是：iOS 从页面给这条安全区上色，**读哪个表面观察不到**，所以三个同色候选要一次全堵上
  —— `<html>`、`<body>`、`theme-color` meta，全部在 `#launch` 退场前保持 `#141310`。
  #125 拆掉了第三个（`applyTheme()` 在模块初始化时就写主题色，那时 W 还在）。
- 修法：meta 初始值 `#141310`；`main.ts` 把颜色**存起来**（`pendingThemeColor`），只有启动脚本
  派发 `pwo-launch-done` 之后才写进去；启动脚本的 `restoreThemeBackground()` 不再删 meta，改成
  加 `launch-done` + 派发事件 + 重入保护。
- **颜色必须由 `main.ts` 决定**，不能让启动脚本自己算 —— 登录页两个主题下都是深色，要 `#050706`，
  启动脚本不知道这回事。
- **收成一个 meta**：#125 留下两个都不带 media 的 `theme-color`，谁赢本身就含糊。
- 时序实测（390px + 4 倍降速）：`45ms W在 → #141310` / `1564ms W在 app已渲染 → #141310` /
  `2150ms W退 → #f4f1ea`；登录页 `1072ms → #050706`；切换按钮来回、刷新后保持、始终 1 个 meta。
- **桌面验不到**：底部安全区只有 iPhone 真机有。等用户确认。
...
## 黑屏 → logo → Overview  `[x]`（2026-09-23，用户重加主屏幕图标后解决，无代码改动）
...
- **已确认解决**：用户删掉图标重新添加后，「直接 w 了」—— 黑屏消失，图标一点就是原生启动图。
  **代码没有任何改动**，#94 遗留的 "re-add icon" 待办就是它，挂了 4 天，代价是每次启动先黑一下。
  教训：交付里写的「需要用户操作」如果没人追，它就不会发生，而症状会被当成性能问题报回来。
...
### T2 — 每人每天额度  `[x]`（2026-09-23，PR #137 已合并）
...
**实际结果**：22 题 0 请求错误、0 安全网触发、0 原则引用。过程中改了三处：

1. **思考根本没关**（真 bug）。「不传 `thinking` 字段 = 关」是错的，两个模型都**默认开启**且思考与正文共用
   `max_tokens` —— 22 题里 3 条返回空、一次问答 24–28 秒（顶着 25 秒上游超时）。显式传
   `thinking: { type: "disabled" }` 之后：0 空回答、3.6 秒。
2. **不再自称**（用户 2026-09-24 提）。几乎每条都以「按 WealthUp 原则」开头，像规章引用自己。
...
### G5 — 「带我过去」按钮  `[x]`（2026-09-24，PR #144，原为缓做的 C 方案）

模型在回答末尾输出 `[[go:ledger]]`，面板变成按钮，**标记不显示**。
**用标记而不是扫正文里的页面名**：扫名字会误伤（「broad market-cap ETFs」会冒出 Open Market 按钮，
有测试守着这条）。标记是页面 id，拿 `pageDirectory` 校验 —— 模型写了不存在的页面就**没有按钮**，
而不是一个点了去不了的死按钮。最多两个；**已经在那一页不给**；跳转后面板保持打开，步骤还在眼前。
实测：从 `#dashboard` 跳到 `#ledger`，面板和答案都在。

**上线**：`firebase deploy --only functions:assistant`（这几轮只改提示词，没动规则）。
