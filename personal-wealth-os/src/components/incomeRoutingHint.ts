...
export function incomeRoutingHint(state: WealthState, input: IncomeRoutingInput): string {
  ...
  const context = preview.receivedBefore > 0.005
    ? `On top of ${money(preview.receivedBefore)} already recorded in ${monthName(preview.monthKey)}, this `
    : "This ";
  ...
  const split = preview.parts.length > 0
    ...
    ? preview.parts.map((part) => `<span style="white-space:nowrap"><strong>${escapeHtml(part.name)}</strong> ${money(part.amount)}</span>`).join(" &middot; ")
    : "";

  const lines = [
    split
      ? `<p class="t-body-sm">${escapeHtml(context)}${money(preview.amount)} goes to ${split}</p>`
      : `<p class="t-body-sm">${escapeHtml(context)}${money(preview.amount)} is not claimed by any layer.</p>`,
  ];

  if (preview.stillShort > 0.005 && preview.essentialName) {
    lines.push(`<p class="t-caption t-muted">${escapeHtml(preview.essentialName)} still needs ${money(preview.stillShort)} this month.</p>`);
  }
  if (split && preview.unclaimed > 0.005) {
    lines.push(`<p class="t-caption t-muted">${money(preview.unclaimed)} is not claimed by any layer.</p>`);
  }
